...
def _grouped_bar(df: pd.DataFrame, metric: str, ylabel: str, title: str, filename: str) -> None:
    pivot = df.groupby(["num_orders", "algorithm"])[metric].mean().unstack()
    ax = pivot.plot(kind="bar", figsize=(9, 5), color=[ALGO_COLORS.get(c, "#888") for c in pivot.columns], edgecolor="white", linewidth=0.5)
...
def plot_all(df: pd.DataFrame) -> None:
    _grouped_bar(df, "total_distance", "Avg total distance",
                 "Total distance by algorithm", "distance_by_algorithm.png")
    _grouped_bar(df, "service_rate", "Avg service rate",
                 "Service rate by algorithm", "service_rate_by_algorithm.png")
    _grouped_bar(df, "runtime_ms", "Avg runtime (ms)",
