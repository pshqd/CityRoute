...
Neighbourhood operators (applied at random each iteration):
  ...
  - insert:          remove an order and reinsert at another position
...
from app.core.models import ConstraintConfig, Route, SAConfig, Scenario
...
def _evaluate(
    ...
    """Build a Route for the given order sequence and return (distance, route)."""
    total_distance = 0.0
    total_time = 0.0
    current = depot
...
def solve_sa(
    ...
    for _ in range(sa_config.max_iterations):
        ...
        _, candidate_route = _evaluate(
            candidate_ids, orders_by_id, depot, constraints
        )
