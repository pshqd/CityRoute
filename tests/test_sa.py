...
def test_sa_better_than_naive(
    scenario_small, loose_constraints, sa_config
):
...
def test_sa_tight_minimises_violations(
    scenario_small, tight_constraints, sa_config
):
    """SA should not increase violations vs naive under tight constraints."""
    from app.algorithms.naive import solve_naive
    from app.core.constraints import count_violations

    naive_route = solve_naive(scenario_small, tight_constraints)
    sa_route = solve_sa(scenario_small, tight_constraints, sa_config)
    assert count_violations(sa_route, tight_constraints) <= count_violations(
        naive_route, tight_constraints
    )
